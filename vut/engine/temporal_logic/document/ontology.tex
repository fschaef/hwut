...
Cognitive Subject. At this origin point, no fixation, no writing, is assumed to
exist; there is only the initial Instantiation of a \textbf{Primordial Event}
$e_\dot$.

This initial Event holds $e_\dot$ two predicates:

\begin{enumerate}

    \item \textbf{Truth ($\top$):} The truth of its own existence (Veritas).

    \item \textbf{Discernible ($\delta_\dot$):} A characteristic that marks its 
...
\textbf{Fixation}. A concept is unavoidably tied to meaning, here the
discernible $\delta_\dot$, so what meaning could the first event carry that is
undeniably true? The only possible absolute true `statement' or information that
could be generated at this point is the statement ``I exist'' of the Noumena.
