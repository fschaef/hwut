...
\begin{formalism}[Event and Discernible]
    ...
    An \textbf{Event} $e$ is the instantiation of a `concept', i.e.\ it is the
    instantiation of information content. It is something that can be
    experienced/perceived, that `informs'. It has no temporal expansion. 

    An Event has two properties, namely the truth of its own existence $\top$
    and a {\it discernible\/} $\delta \in \Delta$, the `information content' by
    means of which it possesses identity.

    \begin{equation}
        e := \langle \top, \delta \rangle
    \end{equation}

    Where $\Delta$ is the space of possible discernibles $\delta$.
...
\begin{formalism}[Atomic Fact]
    ...
    an Event. An atomic fact is the result of a fixation, where no 
...
\begin{formalism}[Synthetic Interstice and Immutable Past]
    ...
    \begin{itemize}
        ...
        no time. Events instantiate and imply further events until they
        converge under the rules of consistency. Anything that happens in the
        Synthetic Interstice is indubitably true but possesses no temporal
        extent.
        \item \textbf{Immutable Past ($H$):} The domain of converged, fixated 
        sets of events (History). 
    \end{itemize}

\end{formalism}

Still in a void of anything that may establish any concept of time, the fixated
content $\delta_\dot$ of the Noumena's ``I exist'' becomes an immutable
atomic fact, thus carrying the information of ``I exist permanently''.
...
\begin{formalism}[The Synthetic Interstice]
    ...
    ``gap in time'' between recorded moments of Past. It is the domain of
    ...
    \begin{itemize}
        ...
        \item \textbf{Historic Construct:} It applies Past as an immutable 
...
rules derived from the Noumena or Past.
...
\section{Immutable Past and the Atomic Fact}
...
\begin{formalism}[Past]

    \textbf{Past} $H_k$ is the immutable, totally ordered sequence of sets
    ...
    Past is the cumulative result of fixation cycles upon the completion of
...
Noumena)—the source of pure Being and \textit{Ontological Truth}. Past,
conversely, is the domain of \textbf{Epistemic Truth}—the sole source of
\textit{True Knowing}.

While Past is relative to the Subject (it is the Subject's specific path
...
\begin{formalism}[Inequality (The Negation of Identity)]
    ...
    This operation is valid in both the Synthetic Interstice and Past.
...
operator on events, unlike immutable Past, where Complementation is valid.
...
Past. It also gives rise to faculties of structure and inertia.
