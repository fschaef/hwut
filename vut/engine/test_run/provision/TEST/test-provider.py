...
from   vut.engine.test_run.provision.core import (Provision,        # noqa: E402
                                                  Supply,
                                                  Run)
from   vut.engine.test_run.provision.stage_acquire \
                                          import StageAcquire       # noqa: E402
from   vut.engine.test_run.provision.stage_build \
                                          import StageBuild         # noqa: E402
from   vut.engine.test_run.provision.stage_execute \
                                          import StageExecute       # noqa: E402
from   vut.engine.test_run.provision.stage_canonicalise \
                                          import StageCanonicalise  # noqa: E402
from   vut.engine.test_run.provision.stage_load \
                                          import StageLoad          # noqa: E402
...
def test_roles():
    ...
    pairs = ((StageAcquire,      I_AcquireProvider),
             (StageBuild,        I_BuildProvider),
             (StageExecute,      I_ExecuteProvider),
             (StageCanonicalise, I_CanonicaliseProvider),
             (StageLoad,         I_LoadProvider))
...
def test_door():
    ...
    build   = StageBuild(configuration)
    execute = StageExecute(configuration)
    try:
        Provision(stage_execute=build)
        refusal = "nothing"
    except AssertionError as x:
        refusal = str(x)
    passed = Provision(stage_execute=execute)
    print("INSPECT: a build provider knocks at the execute slot")
    print("         refused: %r" % refusal)
    print("         the execute provider passes: %s"
          % (passed.stage_execute is execute))
    ok = _check([
        ("I_ExecuteProvider" in refusal and "StageBuild" in refusal,
...
def test_scheme():
    ...
    import dataclasses
    from vut.engine.test_run.provision.multi_execute import MultiExecute
    ...
    queue   = [local, proxy]
    role_db = (I_AcquireProvider, I_BuildProvider, I_ExecuteProvider,
               I_CanonicaliseProvider, I_LoadProvider)
