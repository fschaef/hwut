...
#     choices    = ["every_format", "every_artefact", "no_orphan"]
...
from vut.engine.coverage.readers.reader import registered_tuple, framework_of
...
def _all_files_of(directory):
    """RETURN: list[str], every file directly under 'directory'."""
    if not os.path.isdir(directory): return []
...
def test_no_orphan():
    """
    EVERY FILE UNDER A FORMAT'S DIRECTORY IS EITHER AN ARTEFACT (its
    format's own suffix, opened by 'every_artefact') OR NAMED
    PROVENANCE -- the source a real tool ran to produce an artefact
    beside it. A file that is neither is an ORPHAN: it satisfies
    nobody's suffix, so no test opens it, and it sits indistinguishable
    from provenance while proving nothing about the format it stands
    under. This is the gap 'every_artefact' cannot see by itself --
    that a file was witnessed says nothing about whether it was FILED
    where a reader would ever find it.
    """
    #  PROVENANCE SUFFIXES SEEN IN THIS DIRECTORY TODAY. Not a
    #  universal list of source-file extensions -- new ones are
    #  expected as more formats are witnessed -- but every file that is
    #  not an artefact must end in one of these, or it is unaccounted
    ...
    for name, (fmt, suffix_tuple, _tools) in sorted(_format_db().items()):
        ...
        directory  = os.path.join(REAL, name)
        artefact   = set(_artefacts_of(directory, suffix_tuple))
        orphan_list = [f for f in _all_files_of(directory)
                       if f not in artefact
                       and not f.endswith(provenance_suffix_tuple)]
        pair_list.append(
            (not orphan_list,
             "%-12s every file is an artefact or named provenance%s"
             % (name, "" if not orphan_list
                     else " -- ORPHAN: %s" % ", ".join(orphan_list))))
    _verdict(_check(pair_list),
             "nothing stands unaccounted for between the two categories.")
