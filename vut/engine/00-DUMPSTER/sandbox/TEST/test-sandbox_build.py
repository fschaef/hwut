...
PURPOSE: Verify the supervised BUILD (sandbox_build.py): one door,
         input = configuration (SandboxConfigBuild), output = result
         (SandboxResultBuild) -- the targets ACTUALLY BUILT are
...
'test-sandbox_build-variants.py'.
...
def _build_config(work_dir, target_list, wall=20.0, tool=None):
    """
    RETURN: SandboxConfigBuild, a make-driven build of 'target_list'
            in 'work_dir' under a wall-clock cap.
    """
    return SandboxConfigBuild(
...
def _print_build_diagnostics(label, result):
    ...
    RETURN: None. On a failed build -- nonzero exit, containment, or a
            ...
            never sees this.)
    """
    if result.record.ok and not result.missing_target_list:
        return
    ...
    for line in result.record.stderr_tail.splitlines():
...
async def test_build_classification():
    ...
    async def classify(makefile, target_list, wall=20.0, tool=None):
        """RETURN: SandboxResultBuild of one provoked build."""
        with tempfile.TemporaryDirectory(prefix="vut_build_") as work_dir:
    ...
    ok = _check([
        (contained.record.containment
             is E_Containment.WALL_CLOCK_EXCEEDED,
         "contained build: the record names the resource"),
        (failed.record.stderr_tail != "",
