...
PURPOSE
       THE PORT -- the contract between an ALIGNMENT and whoever shows it.

DESCRIPTION
       Two halves (README 11.1). DOWN carries the alignment out; UP
       carries a human's resolution back. DISPLAY uses DOWN only; MERGE
       uses DOWN then UP.

       THIS MODULE IMPORTS NOTHING OF COMPARE. The alignment -- the
       association run's product, the immutable DisplayInst stream of
       'compare/feeder/ui.py' -- is HANDED IN by the caller, who got it
       from compare's one door ('compare.api'). There is no second road
       to the association: a module that fetched the alignment on the
       caller's behalf was a corridor built in front of that door, and
       it is gone. Here stands only what both sides must agree on: the
       adapter sequence, the intents, the envelopes, and the merge
       dialogue over them.
...
async def merge_session(align, subject_text, nominal_text,
    ...
    ONE ASSOCIATION PER ROUND: 'align(subject_text, working)' is an
    async iterable of DOWN items, HANDED IN by the caller -- typically
    compare's own door with the options applied. Each round calls it
    afresh; nothing carries over between rounds and nothing needs to.
    This dialogue knows the SHAPE of an alignment and nothing about
    where alignments come from (door count: one).
...
async def deliver(item_iterable, adapter, subject_name):
    """
    RETURN: int, how many DOWN items were carried.

    The DOWN half for one subject: open, present every item of
    'item_iterable' -- an alignment the CALLER already obtained from
    compare's door -- close. 'close' runs however the body ended, so a
    driver's resources are released even when presentation raised.
    """
    await _call(adapter, "open", subject_name)
    count = 0
    try:
        async for item in item_iterable:
            await _call(adapter, "present", item)
            count += 1
    finally:
        await _call(adapter, "close")
