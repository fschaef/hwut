...
PURPOSE: THE 'hwut.run.cov' COMMAND LINE -- coverage, as one executable with
         verbs (coverage RATIONALE D-13, D-20).

    hwut.run.cov [<wish>] [--variant=<a>,<b>] [<hwut.run options>]
                                ...
                                language else
    hwut.run.cov gather [--directory=<path>] [--out=<file>]
                                ...
                                directory's 'GOOD/'.
    hwut.run.cov stale BUNDLE [--directory=<path>]
                                which directories of that bundle have
                                MOVED since it was taken
    hwut.run.cov convert [--from <form>] [--to <form>] FILE
    ...
    hwut.run.cov formats            the table of tools, formats and
...
USAGE = "usage: hwut.run.cov [<wish>] | convert [--from binary|text] " \
...
def main(argv=None, write=None, write_bytes=None, demand=None):
    ...
    if not argv or argv[0] not in VERB_TUPLE:
        ...
        from vut.services.run import main as run_main
...
if __name__ == "__main__":
    ...
    sys.exit(guarded("hwut.run.cov", main, sys.argv[1:]))
